# app_streamlit.py — Enhanced Sentiment App (Neutral Band ± around 0.5, upgraded profanity detection, no ELI5)
...
# Extended profanity list (lowercase; non-exhaustive; no slurs)
_PROFANITY = {
    "fuck","fucking","fucker","fucked","motherfucker","mf","wtf",
    ...
    "crap",
    "piss","pissed",
    "damn","goddamn","goddammit","bloody",
    "douche","douchebag",
    "balls","bollocks","bugger","arse"
}

# Map common leetspeak / symbol substitutions to letters
...
def normalize_obfuscation(s: str) -> str:
    # Lower, convert leetspeak, strip non-alphanum, collapse long repeats
    s = s.lower().translate(LEET_MAP)
    s = re.sub(r"[^a-z0-9]+", "", s)
    s = re.sub(r"(.)\1{2,}", r"\1\1", s)  # "fuuuuuck" -> "fuuck"
    return s

def tokenize_raw_chunks(t: str):
    # keep chunks that may include numbers/symbols to catch obfuscations
    return re.findall(r"[a-z0-9@#$*!']+", t.lower())

def find_profanity_tokens(t: str):
    raw = tokenize_raw_chunks(t)
    norm = [normalize_obfuscation(tok) for tok in raw]
    found = [w for w in norm if w in _PROFANITY]
...
def word_count(t: str) -> int:
    # Count alpha tokens only for readability
...
st.caption("Neutral band around P(positive)=0.5, short-text warnings, upgraded profanity detection, and batch processing.")
...
with tabs[3]:
    ...
    st.markdown(
        """
        **Neutral band logic**
        - Compute P(positive) from `predict_proba` (if available) or SVM margin via sigmoid.
        - If P(positive) ∈ [0.5 − band, 0.5 + band] → output **neutral**; else positive/negative.
        - Default band = **±0.05** (adjustable in the sidebar).

        **Profanity detection (upgraded)**
        - Normalizes leetspeak/symbols and collapses long repeats to catch forms like `f@ck`, `f**k`, `fuuuuuck`, `sh1t`.
        - Optional redaction masks matched words in Notes (e.g., `f**k`).

        **Edge cases**
        - If the classifier is not binary or probabilities cannot be computed, the app falls back to the raw label.
        - Short texts trigger a stability warning.

        **Rubric alignment**
        - Functionality & Correctness: probabilistic neutral handling, robust batch processing.
        - Usability & Professionalism: policy sliders, notes, clean UI.
        - Reproducibility: same artifact paths as the training notebook/report.
        """
    )
